...
describe("url builders", () => {
  ...
  it("buildBlogPostPath strips incoming lang and prefixes target", () => {
    expect(buildBlogPostPath("de", "en/awesome-post")).toBe(
      "/de/blog/awesome-post",
    );
    expect(buildBlogPostPath("en", "awesome-post")).toBe(
      "/en/blog/awesome-post",
    );
    expect(buildBlogPostPath("de", "/en/awesome/post")).toBe(
      "/de/blog/awesome/post",
    );
  ...
  it("buildTagPath uses encodeTagPath and prefixes", () => {
    expect(buildTagPath("en", "programming/javascript")).toBe(
  ...
  it("stripLangFromUrlOrId used by blog path handles edge cases (integration)", () => {
    ...
    expect(buildBlogPostPath("en", "/en//nested///post")).toBe(
      "/en/blog/nested/post",
    );
  });
});
